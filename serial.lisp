...
;;; If the object matches the parent class perfectly (no extra data), the parent-class is returned
...
;;; Only supports typedef of first direct superclass (todo: handle inheritance properly. oh boy)
  ...
  (let ((spec (slot-spec/type-def slot-spec)))
    (trivia:match spec
      (nil
       obj)
      (:inherit
       (let* ((first-super (loop named lop
			     for i in (arrow-macros:->> slot-spec
					(slot-spec/class-ref)
	      ...
	      (correct-spec (loop for s in super-specs
			       if (closer-mop:slot-definition-name (slot-spec/ref s))
			       return s
  ...
  (let* ((ref (class-spec/ref class-spec))
    ...
    (loop for s in (class-spec/slot-specs class-spec)
       ...
       do (if aso
	      ...
	      (error "Tried to serialize alist ~a into class ~a, but slot ~a was not fonud"
		     alist
		     ref
		     slot-name)))
    obj))
...
(defun serial->obj (alist class-sym)
  (multiple-value-bind (val found) (gethash (find-class class-sym) *class-specs*)
    (declare (ignore val))
    (if found
